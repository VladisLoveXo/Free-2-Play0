...
export const GameCard = ({ gameData, toggleModal, isOpen }) => {
  ...
  return (
    <div
      className={`game-card ${isOpen ? "opened" : ""}`}
      onClick={() => toggleModal(gameData.id)}
    >
      ...
      <div className="bottom-game-card">
        ...
        <div>
          <div className="description-game-card">
            Студия <span>«{gameData.developer}»</span>
          </div>
          <div className="description-game-card">
            Год релиза: <span>{formateDate(gameData.release_date)}</span>
          </div>
